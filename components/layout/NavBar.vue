    ...
    <ul class="hidden sm:flex align-end items-center gap-0 list-none m-0 p-0">
      ...
      v-for="item in menuItems" :key="item.href" class="nav-item relative flex items-stretch"
        ...
        v-if="item.children"
          ...
          class="absolute top-full left-0 min-w-[250px] max-h-[80vh] overflow-y-auto bg-neutral-900 list-none m-0 transition-all duration-200 ease-in-out"
...
.nav-link {
  @apply flex items-center py-3 px-2 md:px-3 lg:px-6 text-layout-text-on-dark font-medium text-sm lg:text-base;
