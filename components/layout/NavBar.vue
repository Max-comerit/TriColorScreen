...
<template>
  <nav class="flex flex-1 justify-end" :class="{ 'is-touch-device': isTouchDevice }" aria-label="Huvudnavigering">
    <ul class="hidden sm:flex  align-end items-center gap-0 list-none m-0 p-0">
      <li v-for="item in menuItems" :key="item.href" class="nav-item relative flex items-stretch"
        ...
        <button v-if="item.children" type="button" class="nav-link" :class="{ active: isActiveOrParent(item) }"
        ...
        <NuxtLink v-else :to="item.href" class="nav-link" :class="{ active: isActiveOrParent(item) }"
        ...
        <ul v-if="item.children"
          ...
          <li v-for="(child, index) in item.children" :key="child.href" class="dropdown-item flex">
            <NuxtLink :to="child.href" class="dropdown-link" :class="{
    ...
    <div class="visible sm:hidden">
      ...
      <BurgerMenu v-model="isBurgerOpen" side="right" :menuItems="menuItems" />
    </div>
  </nav>
</template>
